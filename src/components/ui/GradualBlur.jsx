...
const useResponsiveDimension = (responsive, config, key) => {
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      const mk = `mobile${key[0].toUpperCase()}${key.slice(1)}`;
      const tk = `tablet${key[0].toUpperCase()}${key.slice(1)}`;
      const dk = `desktop${key[0].toUpperCase()}${key.slice(1)}`;
      let v = config[key];
      if (w <= 480 && config[mk]) v = config[mk];
      else if (w <= 768 && config[tk]) v = config[tk];
      else if (w <= 1024 && config[dk]) v = config[dk];
      setVal(v);
    };
