            ...
            <div className="relative flex flex-1 flex-col z-10">
              <div className="mb-6 flex flex-col gap-1.5">
                <div className="flex justify-between items-start gap-4">
                  <div>
                    <h3 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-cyan-400 transition-colors">
                      {project.name}
                    </h3>
                    <p className="text-sm font-bold text-slate-600 dark:text-slate-400 mt-1">{project.subtitle}</p>
                  </div>
                  <div className="flex gap-2">
                    {project.github && (
                      <motion.a whileHover={{ scale: 1.1, rotate: 5 }} whileTap={{ scale: 0.9 }} href={project.github} target="_blank" rel="noopener" className="p-2 rounded-xl bg-white/40 dark:bg-white/5 border border-slate-300 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-cyan-400 hover:bg-white/80 dark:hover:bg-cyan-500/20 hover:border-blue-500 dark:hover:border-cyan-400/50 hover:shadow-lg transition-all">
                        <Github className="h-4 w-4" />
                      </motion.a>
                    )}
                    {project.live && (
                       <motion.a whileHover={{ scale: 1.1, rotate: -5 }} whileTap={{ scale: 0.9 }} href={project.live} target="_blank" rel="noopener" className="p-2 rounded-xl bg-white/40 dark:bg-white/5 border border-slate-300 dark:border-white/10 text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-cyan-400 hover:bg-white/80 dark:hover:bg-cyan-500/20 hover:border-blue-500 dark:hover:border-cyan-400/50 hover:shadow-lg transition-all">
              ...
              <ul className="text-[13px] sm:text-[14px] font-bold leading-[1.6] text-slate-800 dark:text-slate-300 space-y-2.5 mb-6 flex-1 pr-2 max-h-[200px] overflow-y-auto custom-scrollbar">
                {project.bullets.map((bullet, i) => (
                  ...
                  >
                    <span className="text-blue-600 dark:text-cyan-500 mt-1 text-xs">▹</span>
                    <span>{bullet}</span>
                  </motion.li>
                ))}
              </ul>
                ...
                <div className="flex flex-wrap gap-2">
                  {project.tech.map((t) => (
                    <motion.span
                      ...
                      key={t}
                      className="rounded-md bg-white/50 dark:bg-white/5 px-2.5 py-1 text-[10px] uppercase tracking-wider text-slate-700 dark:text-slate-300 font-black border border-slate-300 dark:border-white/10 cursor-default hover:border-blue-500 dark:hover:border-cyan-400/50 hover:text-blue-600 dark:hover:text-cyan-400 transition-colors shadow-sm"
                    >
                      {t}
                    </motion.span>
                  ))}
      ...
      <div className="mx-auto flex max-w-7xl flex-col gap-16 px-6 lg:px-8">
        ...
        >
          ...
          <p className="text-[16px] text-slate-700 dark:text-slate-400 font-bold max-w-2xl leading-relaxed">
            A showcase of comprehensive full-stack applications built to solve problems with scalable solutions.
          </p>
        </motion.div>
